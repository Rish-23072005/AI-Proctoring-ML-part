...
class STT:
    ...
    def load_model(self):
        """Load the Whisper model, handling local and remote cases."""
        try:
            self.model = WhisperModel(
                ...
                model_dir="path/to/local/model/directory"  # Specify the local directory

            )
        except Exception as e:
            logging.error(f"Failed to load the model: {e}")
            raise RuntimeError(
                "Model loading failed. Please ensure the model is available locally or internet access is enabled."
            )
...
try:
    ...
    while stt.is_listening:
        ...
        if len(last_transcription) > 0:
            ...
            # If user said 'stop', stop the transcription process
